...
-- lua FIWARE lib: sidecar-proxy auth endpoint config handler
local auth_handler = require "fiware.ishare.sp_auth_endpoint_handler"
...
-- Init
local AuthEndpointConfigHandler = BasePlugin:extend()
AuthEndpointConfigHandler.PRIORITY = 1011
AuthEndpointConfigHandler.VERSION = "0.0.1"

function AuthEndpointConfigHandler:new()
  AuthEndpointConfigHandler.super.new(self, "auth-endpoint-config-ishare")
end
...
function AuthEndpointConfigHandler:access(config)
   AuthEndpointConfigHandler.super.access(self)
   kong.log.debug(" *** Sidecar-Proxy Auth Endpoint Config Service plugin access() function entered ***")
   ...
   -- Call handler
   local err = auth_handler.handle_request(proxy_config, req_dict)
   if err then
      -- Access not granted
      return handle_error(401, err)
   end
...
return AuthEndpointConfigHandler
